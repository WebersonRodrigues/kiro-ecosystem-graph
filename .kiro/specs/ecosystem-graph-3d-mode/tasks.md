...
- [x] 1. Infraestrutura e abstração do renderizador
  - [x] 1.1 Adicionar dependência `3d-force-graph` ao projeto
    - Adicionar `"3d-force-graph": "^1.73.0"` em `dependencies` no `package.json`
    - Executar install para gerar lock file atualizado
    - _Requisitos: 5.1_

  - [x] 1.2 Criar `src/webview/media/renderer-manager.js` com interface unificada
...
- [x] 2. Refatorar `webview.js` para usar o Renderer Manager
  - [x] 2.1 Extrair lógica de criação de instância do `webview.js` para o `renderer-manager.js`
  ...
  - [x] 2.2 Atualizar `webviewProvider.ts` para incluir novos scripts e CSP
...
- [x] 3. Checkpoint — Garantir que o modo 2D continua funcionando
...
- [x] 4. Toggle de modo no painel de configurações
  - [x] 4.1 Adicionar controle "Render Mode" no `settings-panel.js`
...
- [x] 5. Implementação da renderização 3D
  - [x] 5.1 Criar `src/webview/media/renderer-3d.js` com configuração de nós 3D
  ...
  - [x] 5.2 Implementar labels como billboard sprites no modo 3D
  ...
  - [x] 5.3 Configurar partículas direcionais e fundo escuro no modo 3D
...
- [x] 6. Checkpoint — Validar renderização 3D básica
  - Garantir que todos os testes passam, perguntar ao usuário se surgirem dúvidas.
  - O grafo deve renderizar em 3D com nós diferenciados por geometria e labels visíveis.

- [x] 7. Paridade de funcionalidades entre modos
  - [x] 7.1 Implementar filtros de tipo no modo 3D
  ...
  - [x] 7.2 Implementar tooltip no hover para modo 3D
  ...
  - [x] 7.3 Implementar click em nó para abrir arquivo no modo 3D
    - Usar `onNodeClick` callback (API idêntica ao 2D)
    - Disparar mensagem `{ type: 'openFile', filePath: node.path }` via postMessage
    - _Requisitos: 4.3_

  - [x] 7.4 Implementar highlight de propagação no modo 3D
  ...
  - [x] 7.5 Garantir painel de estatísticas visível no modo 3D
...
- [x] 8. Configurações de força no modo 3D
  - [x] 8.1 Aplicar configurações de força à simulação 3D em tempo real
...
- [x] 9. Tratamento de erros e fallback
  - [x] 9.1 Implementar fallback automático para 2D quando WebGL falha
  ...
  - [x] 9.2 Implementar lazy loading do script `3d-force-graph`
  ...
  - [x] 9.3 Implementar limpeza de memória na destruição de instância 3D
